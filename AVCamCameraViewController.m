...
{
    NSLog( @"Entering configureSession" );
	if ( self.setupResult != AVCamSetupResultSuccess ) {
		return;
	}
	...
	*/
	...
	if ( ! videoDevice ) {
        NSLog( @"Back dual cameras are not available." );
		// If the back dual camera is not available, default to the back wide angle camera.
		videoDevice = [AVCaptureDevice defaultDeviceWithDeviceType:AVCaptureDeviceTypeBuiltInWideAngleCamera mediaType:AVMediaTypeVideo position:AVCaptureDevicePositionBack];
		
		// In some cases where users break their phones, the back wide angle camera is not available. In this case, we should default to the front wide angle camera.
		if ( ! videoDevice ) {
            NSLog( @"Back wide angle camera not available" );
    ...
    NSLog( @"self.backgroundRecordingID = %lo", self.backgroundRecordingID );
	[self.session commitConfiguration];
    NSLog( @"Ending configureSession");
}
...
- (IBAction)toggleCatureMode:(UISegmentedControl *)captureModeControl
{
...
//			self.recordButton.enabled = self.captureModeControl.selectedSegmentIndex == AVCamCaptureModeMovie;
            ...
            [self.delegate shouldEnableRecordUI:self.captureModeControl.selectedSegmentIndex == AVCamCaptureModeMovie];
...
{
    NSLog( @"Entered toggleMovieRecording");
	/*
...
//	self.recordButton.enabled = NO;
	...
	*/
	AVCaptureVideoOrientation videoPreviewLayerVideoOrientation = self._previewView.videoPreviewLayer.connection.videoOrientation;
	NSLog( @"Step 1(determine videoPreviewLayerVideoOrientation): %ld", (long)videoPreviewLayerVideoOrientation );
	dispatch_async( self.sessionQueue, ^{
        NSLog( @"Step 2(create sessionQueue)." );
		if ( ! self.movieFileOutput.isRecording ) {
            NSLog( @"Step 3(no current recording in progress)." );
			if ( [UIDevice currentDevice].isMultitaskingSupported ) {
                NSLog( @"Step 4(device can multitask)." );
				/*
				...
				*/
                ...
                NSLog( @"Step 4a(set backgroundRecordingID): %lu", (unsigned long)self.backgroundRecordingID );
			}
            NSLog( @"Step 5(set orientation)." );
			// Update the orientation on the movie file output video connection before starting recording.
			AVCaptureConnection *movieFileOutputConnection = [self.movieFileOutput connectionWithMediaType:AVMediaTypeVideo];
			movieFileOutputConnection.videoOrientation = videoPreviewLayerVideoOrientation;
            NSLog( @"Step 6(start recording to temporary file)." );
			// Start recording to a temporary file.
			NSString *outputFileName = [NSUUID UUID].UUIDString;
			NSString *outputFilePath = [NSTemporaryDirectory() stringByAppendingPathComponent:[outputFileName stringByAppendingPathExtension:@"mov"]];
            NSLog( @"Step 7(temp filename is %@:", [NSURL fileURLWithPath:outputFilePath] );
			[self.movieFileOutput startRecordingToOutputFileURL:[NSURL fileURLWithPath:outputFilePath] recordingDelegate:self];
            NSLog( @"Step 7a(start recording to temporary file.  State is: %d)", (int)self.movieFileOutput.isRecording );
		}
		else {
            NSLog( @"Step 8(a prior recording was in progress, stop recording):" );
			[self.movieFileOutput stopRecording];
		}
	} );
    NSLog( @"Leaving toggleMovieRecording");
}

- (void)captureOutput:(AVCaptureFileOutput *)captureOutput didStartRecordingToOutputFileAtURL:(NSURL *)fileURL fromConnections:(NSArray *)connections
{
    NSLog( @"Entered captureOutput didStartRecordingToOutputFileAtURL");
...
{
    NSLog( @"Entered captureOutput didFinishRecordingToOutputFileAtURL");
	/*
...
//			self.recordButton.enabled = isSessionRunning && ( self.captureModeControl.selectedSegmentIndex == AVCamCaptureModeMovie );
            ...
            [self.delegate shouldEnableRecordUI:isSessionRunning && ( self.captureModeControl.selectedSegmentIndex == AVCamCaptureModeMovie )];
